...
void setup() {
  ...
  Serial.println("--- SYSTEME Maitre Demarre ---");

  while (digitalRead(statePin) == LOW) {
    Serial.println("--- En attente connexion module Esclave ---");
    delay(500);
  }

  Serial.println("--- Connexion module Esclave etablie ---");
  BTSerial.println("CONNECTE");
}
...
void loop() {
  ...
  if (digitalRead(statePin) == HIGH) {
    ...
    digitalWrite(13, LOW);
    Serial.println("Erreur : Module non connecte.");
